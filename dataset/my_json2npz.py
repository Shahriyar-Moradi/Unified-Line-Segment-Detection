...
def save_npz(prefix, image, lines, centers, cfg):
    ...
    if len(lines.shape) ==1:
        ...
        # centers = centers * sx # 1D case
        # centers[:,0] = centers[:,0]* sx
...
# Concatenate 
    if len(lines)==0:
        start = np.array([0]) # 1D array
        end = np.array([0]) # 1D array 
        juncs = np.concatenate((start, end))
    else:
        start = lines[0]
        end = lines[-1]
        juncs = np.concatenate((start, end))
    ...
    if n_pts % 2 == 1:
        if len(lines.shape) == 1:
            # lines is 1D, skip delete
            pass
        else:  
            # lines is 2D, can delete 
