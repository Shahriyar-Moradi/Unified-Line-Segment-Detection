...
def insert_line(image, lines, color, thickness=0):
    ...
    for pts in pts_list:
        ...
        try:
      # Draw polylines
            cv2.polylines(image, np.int32([pts]), False, color=color, thickness=thickness) 
    
        except Exception as e:
            print("Error drawing polylines: ", e)
        # pts = np.round(pts).astype(np.int32)
        # cv2.polylines(image, np.int32([pts]), isClosed=False, color=color, thickness=thickness)
